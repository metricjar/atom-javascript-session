...
if [[ $1 == "test" ]]; then
    aws --region us-east-1 s3 rm --recursive s3://cdn.atom-data.io/js/session/test/
    aws --region us-east-1 s3 cp dist/ s3://cdn.atom-data.io/js/session/test/ --recursive --acl public-read
    aws cloudfront create-invalidation --distribution-id EETANK4D76IR0 --paths /session/test/*
else
